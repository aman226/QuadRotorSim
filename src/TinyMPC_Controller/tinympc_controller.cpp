// #include <TinyMPC_Controller/tinympc_controller.hpp>
// #include <iostream>
...
//         {
//             (solver->work->d.col(i)) = solver->cache->Quu_inv * (solver->work->Bdyn.t()) * solver->work->p.col(i + 1) + solver->work->r.col(i));
//             (solver->work->p.col(i)) = solver->work->q.col(i) + solver->cache->AmBKt*(solver->work->p.col(i + 1)) - (solver->cache->Kinf.t())*(solver->work->r.col(i)); // + solver->cache->coeff_d2p * solver->work->d.col(i); // coeff_d2p always appears to be zeros (faster to comment out)
//         }
//     }
...
//         {
//             (solver->work->u.col(i)) = -solver->cache->Kinf*(solver->work->x.col(i)) - solver->work->d.col(i);
//             // solver->work->u.col(i) << .001, .02, .3, 4;
//             // DEBUG_PRINT("u(0): %f\n", solver->work->u.col(0)(0));
//             // multAdyn(solver->Ax->cache.Adyn, solver->work->x.col(i));
//             (solver->work->x.col(i + 1)) = solver->work->Adyn*(solver->work->x.col(i)) + solver->work->Bdyn*(solver->work->u.col(i));
//         }
//     }
...
//         {
//             solver->work->znew = (arma::abs(solver->work->u_max - solver->work->u_min) + solver->work->u_max + solver->work->u_min) / 2 - (arma::abs(solver->work->u_max - solver->work->u_min) - solver->work->znew) / 2;
//         }

//         // Box constraints on state
//         if (solver->settings->en_state_bound)
//         {
//             solver->work->vnew = (arma::abs(solver->work->x_max - solver->work->x_min) + solver->work->x_max + solver->work->x_min) / 2 - (arma::abs(solver->work->x_max - solver->work->x_min) - solver->work->vnew) / 2;
//         }
//     }
...
//         solver->work->q = -(solver->work->Xref.each_col() * solver->work->Q);
//         (solver->work->q) -= solver->cache->rho * (solver->work->vnew - solver->work->g);
//         solver->work->p.col(NHORIZON - 1) = -(solver->work->Xref.col(NHORIZON - 1).t()*(solver->cache->Pinf));
